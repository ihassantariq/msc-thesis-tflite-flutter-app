...
class DNNPage extends StatefulWidget {
  const DNNPage({Key? key, required this.title}) : super(key: key);

  final String title;

  @override
  _DNNPageState createState() => _DNNPageState();
}

class _DNNPageState extends State<DNNPage> {
  ...
  Future<void> SetStates() async {
    _model_results =
        await _inferenceHelper.infernce_to_DNN_Model(_csv_data.features[index]);
    setState(() {
      // print(_model_results.indexes);
      // print(_csv_data.map_indexes_to_labels);
